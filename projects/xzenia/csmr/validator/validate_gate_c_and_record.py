...
GATE_C = WORKSPACE / 'projects/xzenia/csmr/validator/gate_c_contradiction.py'
...
def main():
    if len(sys.argv) != 2:
        print('usage: validate_gate_c_and_record.py <proposal.json>', file=sys.stderr)
        raise SystemExit(2)
    ...
    run = subprocess.run(['python3', str(GATE_C), str(proposal_path)], capture_output=True, text=True)
    report = json.loads(run.stdout.strip())
    report_path = REPORT_DIR / f'{proposal_id}-contradiction-report.json'
    report_path.write_text(json.dumps(report, indent=2) + '\n')

    conn = sqlite3.connect(DB)
    try:
        if run.returncode == 0:
            set_status(conn, proposal_id, 'validated_gate_c')
            print(json.dumps({'ok': True, 'proposal_id': proposal_id, 'status': 'validated_gate_c', 'report': str(report_path)}, indent=2))
            return
        rejection = {
            'proposal_id': proposal_id,
            'failed_gate': 'C',
            'violation_type': 'CausalContradiction',
            'violated_field': report.get('contradicted_failure') or 'downstream_failure_probability_delta',
            'safe_range': {'max_delta': 0.05},
            'recommended_delta': 0.05,
            'detail': report,
        }
        rejection_path = Path('/tmp') / f'{proposal_id}-gate-c-rejection.json'
        ...
        set_status(conn, proposal_id, 'rejected_gate_c')
        print(json.dumps({'ok': False, 'proposal_id': proposal_id, 'status': 'rejected_gate_c', 'report': str(report_path)}, indent=2))
        raise SystemExit(1)
    finally:
        conn.close()
